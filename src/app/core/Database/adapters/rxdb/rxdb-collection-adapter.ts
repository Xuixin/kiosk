  ...
  implements CollectionAdapter<T>
{
  ...
  async query(request: QueryRequest<T>): Promise<QueryResult<T>> {
    try {
      ...
      // Apply sorting
      // Note: RxDB sort may need to be chained differently
      // For now, we'll apply the first sort only
      if (request.sort && request.sort.length > 0) {
        const firstSort = request.sort[0];
        query = (query as any).sort(firstSort.field as any) as any;
        if (firstSort.direction === 'desc') {
          query = (query as any).desc() as any;
        }
      }
